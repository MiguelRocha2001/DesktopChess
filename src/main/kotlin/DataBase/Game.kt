 ...
 */
fun String.toPlayerOrNull() = if (isValidPlayerId(this)) Game(this) else null
 ...
 */
data class Moves(val game: Game, val content: String) {
    init { require(content.isNotBlank()) }
    fun getMove() = content
    override fun toString() = "${game.id}: $content"
}